...
| `FileNotFoundError: <name>: neither a file nor the name of a train/imitate run under runs` from `watch` / `evaluate` / `train` | `SNAKE_MODEL_PATH` is a run name but `runs_dir` (default `runs/`, relative to the working directory) holds no `<ts>_train_<name>` / `<ts>_imitate_<name>` directory, e.g. when running from a worktree | pass `--set runs_dir=<folder with the runs>` or the checkpoint's `.zip` path |
