...
| [linyiLYi/snake-ai](https://github.com/linyiLYi/snake-ai) | Apache-2.0 | ideas only: MaskablePPO for board-filling snake, the body-gradient observation channel (also the tail-to-neck shading of the `play`/`watch` window), decaying learning rate and clip range, the `4 * cells` starvation cap |
| [DLR-RM/rl-baselines3-zoo](https://github.com/DLR-RM/rl-baselines3-zoo) | MIT | idea only (not code): the `enjoy` loop of [rl_zoo3/enjoy.py](https://github.com/DLR-RM/rl-baselines3-zoo/blob/master/rl_zoo3/enjoy.py) (predict, step, render) is the shape of the `watch` phase |
...
| [pygame-community/pygame-ce](https://github.com/pygame-community/pygame-ce) | LGPL-2.1 (its package metadata, https://pypi.org/project/pygame-ce/) | library dependency: window, keyboard events, `surfarray` and font of the `play` / `watch` phases |
| [python-pillow/Pillow](https://github.com/python-pillow/Pillow) | MIT-CMU (its package metadata, https://pypi.org/project/pillow/) | library dependency installed by matplotlib: writes the animated GIF of `watch --set watch_gif=1` |
